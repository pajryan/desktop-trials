...
function browserRequire(identifier, callback, compiler) {
	if (identifier instanceof Array) {
		var modules = new Array();
		var modcount = identifier.length;
		for (var index = 0; index < identifier.length; index++) {
			(function(id, i) {
				modules.push(browserRequire(id, callback&&function(mod) {
					modules[i] = mod;
	...
	function onLoad() {
		...
		if (request.status != 200)
			//PJR commenting this out.  This seems to trigger even though everything is fine...
...
if (window.browserRequire !== undefined)
	throw new SmoothieError('\'browserRequire\' already defined in global scope');

try {
	Object.defineProperty(window, 'browserRequire', {'value':browserRequire});
	Object.defineProperty(window.browserRequire, 'resolve', {'value':resolve});
	Object.defineProperty(window.browserRequire, 'path', {'get':function(){return requirePath.slice(0);}});
}
catch (e) {
	...
	window.browserRequire.resolve = resolve;
	window.browserRequire.path = requirePath.slice(0);
}
...
function /*load*/(module/*, cache, pwd, source*/) {
	...
	if (typeof module.id !== 'string')
		for (id in module)
			arguments[1]['$'+browserRequire.resolve(id).id] = module[id].toString();
	arguments[2].shift();
}
